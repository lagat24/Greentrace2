# 🌱 GreenTrace – Tree Planting Competition Platform
**GreenTrace** is an interactive web platform designed to promote tree planting through friendly competitions.  
Users can sign up, plant virtual or real trees, upload pictures, and track their progress on a leaderboard — making environmental action fun, transparent, and rewarding.


## 🌍 Project Overview 
GreenTrace empowers individuals and communities to participate in sustainability efforts by tracking tree-planting activities digitally. Users can register, upload photos of planted trees, and view their progress on a dynamic dashboard and leaderboard. The system uses AI-based verification to ensure transparency and authenticity.

## 🌱 Sustainable Development Goals (SDGs)

**GreenTrace supports key United Nations Sustainable Development Goals, especially SDG 13 (Climate Action) and SDG 15 (Life on Land).**  
By enabling individuals and communities to track, verify, and showcase tree-planting activities, GreenTrace strengthens environmental stewardship, encourages reforestation, and increases transparency in climate-positive actions. The platform empowers users to contribute to carbon-reduction efforts while supporting accountability and data-driven decision-making for sustainability projects.

---

## 🚀 Features

### 🔐 User Signup & Authentication
- Secure account creation and login
- Individual progress dashboards
- Personalized user experience

### 🌳 Tree Upload & Picture Submission
- Record each tree planted.  
- Upload a photo for verification.
- Ai photo verification.  
- Include location details (latitude & longitude) for mapping.

### 🏆 Leaderboard
- Displays top users with the most trees planted.  
- Encourages friendly competition and motivation to plant more.

### 📊 Dashboard & Progress Tracking
- View total trees planted and planting history.  
- Visual progress tracking for engagement and accountability.

---

## How We Use AI
GreenTrace integrates computer vision to verify tree planting photos.
The AI analyzes:
- Soil disturbance
- Leaf and plant patterns
- Environmental consistency
- Time and metadata indicators

This ensures uploaded photos represent genuine environmental contributions, strengthening the platform’s trust, transparency, and impact.

___

## 🌿 Benefits
- **Environmental Impact:** Promotes real-world tree planting and supports climate action.  
- **Gamification:** Leaderboard and achievements make sustainability engaging and fun.  
- **Community Engagement:** Connects users and encourages collaborative eco efforts.  
- **Transparency:** Photo and location verification ensure genuine contributions.

---

## 🧠 Tech Stack
- **Frontend:** HTML, CSS, JavaScript  
- **Backend:** Node.js, Express.js
- **AI:** Tensorflow
- **Database:** SQL (Users, Trees, Leaderboard tables)  
- **Hosting:**  
  - Frontend – Netlify  
  - Backend – Render / Heroku (or local testing)

---

## 🌐 Links / Live Demo
- **Live link (Netlify):** https://greentrace2.netlify.app/  
- **Backend (render):** https://greentrace-t95w.onrender.com/
- **Pitch deck (canva):** https://www.canva.com/design/DAG1qMMYZ5Y/P8ELJWTsX7YdwQPrFn_N2g/view?utm_content=DAG1qMMYZ5Y&utm_campaign=designshare&utm_medium=link2&utm_source=uniquelinks&utlId=h6b9e9e70d6
---

## 🧩 Summary
GreenTrace combines **technology**, **gamification**, and **environmental awareness** to inspire real-world impact.  
This submission demonstrates key features like signup, tree uploads, leaderboard, and progress tracking — making it a strong and scalable foundation for hackathons, community projects, or environmental campaigns.

---
